...
export default function InspectionPart1({ inspectionId, inspectionData, isEmbedded, onCreated }) {
    // If in Edit Mode
    ...
    const {
        ...
        setEdaaDetection,
        loading
    } = useInspectionPhotoViewModel(onCreated);
            ...
            <div>
                ...
                >
                    <div className="flex items-center mb-3 gap-2">
                        <span className="font-bold text-blue-200">{globalNumber}.0</span>
                        <input
                            type="text"
                    ...
                    {unit.group?.map((child, childIdx) => (
                        <div key={child.id} className="relative mt-2">
                            <DraggablePhoto photo={child} parentUnit={unit} />
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto">
                ...
                {selectedEquipment && (
                    <>
                        ...
                        {/* Footer / Next Step */}
                        <div className="flex flex-row justify-end items-end mt-8 gap-3">
                            {/* Left: EDAA Detection Switch */}
                            {/* <div className="flex items-center gap-3 bg-gray-800 p-4 rounded-xl border border-gray-700 w-fit">
                                <span className="text-blue-100 font-medium whitespace-nowrap">EDAA Detection System :</span>
                                <button
                                    onClick={() => setEdaaDetection(!edaaDetection)}
                                    className={`w-14 h-7 flex items-center rounded-full p-1 transition-colors duration-300 ${edaaDetection ? "bg-green-500" : "bg-gray-600"
                                        }`}
                                >
                                    <div className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${edaaDetection ? "translate-x-7" : "translate-x-0"
                                        }`} />
                                </button>
                                <span className={`text-sm font-bold ${edaaDetection ? "text-green-400" : "text-gray-400"}`}>
                                    {edaaDetection ? "ON" : "OFF"}
                                </span>
                            </div> */}

                            {/* Right: Validation & Next Step */}
            ...
            {loading && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999]">
                    <div className="bg-gray-800 p-8 rounded-2xl flex flex-col items-center">
                        ...
                        <h3 className="text-xl font-bold text-white">Saving Inspection...</h3>
                        <p className="text-gray-400 mt-2">Uploading photos and creating records.</p>
